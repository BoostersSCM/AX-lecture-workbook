...
export const SESSIONS = [
  {
    ...
    blocks: [
      ...
      { type: 'head', text: '실습 1 — 연결의 경계 확인하기' },
      { type: 'note', text: '흔한 오해 하나를 실험으로 깹니다 — **"같은 워크스페이스니까 다 읽히겠지."** 아닙니다. 이 워크북의 봇은 **연결 추가(초대)된 페이지만** 읽을 수 있습니다. 아래 작업대에서 ① 내 「AX 실습장」 복제본(연결함 → 성공)과 ② 연결하지 않은 아무 페이지(→ 실패)를 연달아 불러와서, 되는 것과 안 되는 것을 직접 확인합니다. 3회차에서 가장 많이 만나는 에러가 바로 이 경계입니다.' },
      { type: 'panel', id: 's1-notion' },
      { type: 'field', key: 's1.source_list', kind: 'textarea', rows: 5, required: true,
        label: '경계 실험 기록 — 되는 페이지와 안 되는 페이지',
        hint: '성공한 페이지(복제본)와 실패한 페이지의 이름, 그리고 실패했을 때 나온 문구를 그대로 적습니다. "왜 실패했는가"를 한 줄로 덧붙입니다.' },
...
아직 본문 내용은 읽지 마.`
  ...
  s1a: {
    session: 1, title: '참고(선택) — 내 접근 범위 목록 만들기',
    note: '개인 AI 도구에 노션 커넥터를 붙여 쓰는 경우에만 해당하는 선택 실습입니다. 워크북 실습 1은 프롬프트 없이 작업대의 경계 실험 카드로 진행합니다.',
...
export const PROMPT_HELP = {
  ...
  s1a: {
    summary: '개인 AI 도구(노션 커넥터)를 쓰는 경우의 선택 실습입니다. 워크북에서는 실습 1의 경계 실험 카드가 같은 배움을 실패 경험으로 대신합니다.',
    terms: [['접근 불가', '권한이 없거나 연결(초대) 범위에 포함되지 않아 읽을 수 없는 상태입니다. 워크스페이스 멤버라는 사실만으로는 열리지 않습니다.'], ['연결 추가', '페이지 ··· 메뉴에서 앱(봇)을 그 페이지에 초대하는 절차입니다. 봇의 접근 범위는 이 초대 목록이 전부입니다.']],
    steps: ['워크북에서는 경계 실험 카드로 성공(복제본)과 실패(미초대 페이지)를 직접 봅니다.', '개인 AI 도구가 있다면 이 프롬프트로 내 계정 기준의 접근 목록을 뽑아볼 수 있습니다.', '실패 문구는 지우지 말고 기록으로 남깁니다 — 3회차에서 같은 에러를 만났을 때의 해결 열쇠입니다.'],
  },
  s1b: {
